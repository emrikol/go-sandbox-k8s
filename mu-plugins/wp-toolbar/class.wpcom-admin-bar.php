...
class WPCOM_Admin_Bar {
	 ...
	 * @var bool
	 */
	public $changed_locale = false;

	/**
	 * @var bool
	 */
	public $need_to_change_locale = false;
	...
	public function initialize() {
		...
		if ( is_user_logged_in() ) {
			...
			$this->user->blogs = get_ordered_blogs_of_user( get_current_user_id(), true, true );
			$has_localize_link = has_filter( 'option_home', 'localize_link' );

			if ( $has_localize_link ) {
				remove_filter( 'option_home', 'localize_link' );
			}
			...
			if ( $has_localize_link ) {
				add_filter( 'option_home', 'localize_link' );
			}

			// Use get_user_lang_code() except in the Network Admin which we also want to be in English.
			$this->user->locale = get_user_locale();
			if ( is_network_admin() || ! $this->user->locale ) {
				$this->user->locale = 'en';
			}
		}
		...
		// Enqueue RTL instead of regular if user lang pref is RTL and blog is not RTL.
		// Dequeue style if user lang pref is not rtl and blog is.
		$dir = get_user_text_direction();
		if ( 'rtl' == $dir && $dir != get_bloginfo( 'text_direction' ) ) {
			wp_dequeue_style( 'admin-bar' );
			wp_enqueue_style( 'admin-bar-rtl', 'http://s.wordpress.com/wp-includes/css/admin-bar-rtl.css', array(
				'dashicons',
			) );
		} elseif ( 'rtl' != $dir && 'rtl' == get_bloginfo( 'text_direction' ) ) {
			global $wp_styles;
			if ( isset( $wp_styles->registered['admin-bar'] ) ) {
				$wp_styles->registered['admin-bar']->extra['rtl'] = false;
			}
		}

		add_action( 'admin_bar_menu',            array( $this, 'load_user_locale_translations' ), -1 );
		add_action( 'wp_after_admin_bar_render', array( $this, 'unload_user_locale_translations' ), 99 );
	...
	final protected function _render( $root ) {
		...
		$class = 'nojq nojs ' . get_user_text_direction();
		if ( $is_IE ) {
...
if ( ! empty( $node->meta['rel'] ) ) :
	...
	public function load_user_locale_translations() {
		return wpcom_switch_to_locale( $this->user->locale );
	}

	public function unload_user_locale_translations() {
		return wpcom_restore_current_locale();
	}
}
